 ...
 *      after that, any instance that refers to that buffer will TRY to own the buffer at any call, even read
 * ones.
 *
 *
...
namespace cerberus
{
    ...
    class CERBERUS_EXPORT SharedByteBuffer
    {
       ...
       public:
        ...
        // Construct a SharedByteBuffer from a c-string. The string must end with a \0, that will NOT be
        // copied
        SharedByteBuffer(const char* str);

        // Destroy the SharedByteBuffer instance and deallocate the memory ONLY if not used by another
        // ByteBuffer
